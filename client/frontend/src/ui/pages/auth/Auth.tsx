...
export default function Auth({ isLoginPage }: LoginProps) {
    ...
    return (
        <>
            ...
            {!isPending && breakpoint.includes("lg") && (
                <>
                    {isLogin ? (
                        <LoginComponents loginData={loginData} handleLoginClick={handleLoginClick} handleRegisterClick={handleRegisterClick} />
                    ) : (
                        <RegisterComponents registerData={registerData} handleLoginClick={handleLoginClick} handleRegisterClick={handleRegisterClick} />
                    )}
                </>
            )}
            {!isPending && breakpoint.includes("md") || breakpoint.includes("sm") && (
                <>
                    {isLogin ? (
                        <ResponsiveLoginComponents loginData={loginData} handleLoginClick={handleLoginClick} handleRegisterClick={handleRegisterClick}/>
                    ) : (
                        <ResponsiveRegisterComponents registerData={registerData} handleLoginClick={handleLoginClick} handleRegisterClick={handleRegisterClick}/>
                    )}
                </>
            )}
        </>
...
function LoginComponents({ loginData, handleLoginClick, handleRegisterClick }) {
    return (
...
function ResponsiveLoginComponents({ loginData, handleLoginClick, handleRegisterClick }) {
    return (
        <div className="flex flex-wrap mx-auto items-stretch justify-center my-28">
            <div className="flex flex-col bg-purple-900 w-10/12 p-12 justify-center items-center rounded-tl-2xl rounded-tr-2xl">
                <img src="/assets/logo.svg" alt="logo" width={120} />
                <h1 className='font-black text-purple-200 lg:text-4xl text-center'>{loginData && loginData[0].name}</h1>
                <p className='text-slate-50 lg:text2xl'>{loginData && loginData[1].name}</p>
            </div>
            <div className="flex flex-col min-h-full bg-white w-10/12 justify-start items-center rounded-br-2xl rounded-bl-2xl p-3">
                <ButtonGroup style={{display: 'flex', flexWrap: 'nowrap', marginBottom: '40px', width: '100%', alignItems: 'stretch'}}>
                    <Button variant="primary" style={{ borderRadius: "15px 0 0 15px", width: "100%"}}  onClick={handleLoginClick}>Login</Button>
                    <Button variant="primary" style={{ borderRadius: "0 15px 15px 0", width: "100%", borderLeft: "1px solid rgb(233 213 255)"}} className="w-1/2" onClick={handleRegisterClick}>Register</Button>
                </ButtonGroup>
...
function ResponsiveRegisterComponents({ registerData, handleLoginClick, handleRegisterClick }) {
    return (
        <div className="flex flex-wrap mx-auto items-stretch justify-center my-28">
            <div className="flex flex-col bg-purple-900 w-10/12 p-12 justify-center items-center rounded-tl-2xl rounded-tr-2xl">
                <img src="/assets/logo.svg" alt="logo" width={120} />
...
function RegisterComponents({ registerData, handleLoginClick, handleRegisterClick }) {
    return (
        <div className="flex flex-nowrap mx-auto items-stretch justify-center my-28">
            <div className="flex flex-col bg-purple-900 w-2/5 p-12 justify-center items-center rounded-tl-2xl rounded-bl-2xl">
                <img src="/assets/logo.svg" alt="logo" width={120} />
